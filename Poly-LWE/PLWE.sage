...
def random_vec(q, dim):
    """random vector trên ZZ_q độ dài dim"""
...
########################## PLWE
samplerGauss_Z = DiscreteGaussianDistributionIntegerSampler(sig)
######### lấy mẫu 
def initiate_sampler_Pq():
    """Gauss trên Z"""
    global samplerGauss_Z 
    print("Initiating Sampler on Pq")
    samplerGauss_Z = DiscreteGaussianDistributionIntegerSampler(sig)
    return True
...
def call_sample_Pq():
    """Gauss trên P_q"""
    # S := P_q
    return S([samplerGauss_Z() for _ in range(n)])

def create_secret_Pq():
    global secret
    """Create secret PLWE on P_q"""
    secret_vec = random_vec(q, n)
    secret = S(list(secret_vec))
    return True

def secret_sub1_mod_q():
    global lift_s
    poly_in_ZZ_sec = secret.lift().change_ring(ZZ)
    lift_s = Zq(poly_in_ZZ_sec.subs(x=1))
    print("Storing the secret mod q.")
    print("The secret is ", secret)
    print("s(1) mod q = ", lift_s)
    return True

def get_sample_Pq(secret, qval):
    """Tạo các cặp mẫu (a, b) PLWE"""
    e = call_sample_Pq()
    
    a_vec = random_vec(qval, n) # create a uniformly randomly poly in Pq
    a = S(list(a_vec))
...
def create_samples_Pq(numsampsval):
    """Create {numsampsval} samples PLWE [a,b]"""
    global samps, numsamps
    samps = []
    print("Creating samples PLWE...")
    for i in range(numsampsval):
        samp = get_sample_Pq(secret, q)
        samps.append(samp)
    numsamps = len(samps)
    print(f"Done creating {len(samps)} samples PLWE.")
    return True

def attack2_PLWE(numsamps, reportrate, quickflag = False):
    print("")
    print("")
    print(f"******************* Beginning ALGORITHM 2 on PLWE with {numsamps} samples ********************")
    #numsamps = len(samps)
    a = [ 0 for i in range(numsamps)]
    b = [ 0 for i in range(numsamps)]
    
    print("Moving samples to ZZ_q.")
    for i in range(numsamps):
        sample = samps[i]
        # .lift() biến đa thức trong S (Zq[z]/f[z]) thành đa thức trong R (Zq[x])
        # vì R định nghĩa trên Zq, ta cần chuyển hệ số về ZZ để subs an toàn
        poly_in_ZZ_a = sample[0].lift().change_ring(ZZ)
        a[i] = Zq(poly_in_ZZ_a.subs(x=1))
        
        poly_in_ZZ_b = sample[1].lift().change_ring(ZZ)
        b[i] = Zq(poly_in_ZZ_b.subs(x=1))
    ...
    for i in range(2):
        if i == 0:
            # ROUND 1: Kiểm tra xem bí mật thực sự vượt qua được bao nhiêu mẫu
            print("!!!!! ROUND 1: !!!!! First, checking how many samples the secret survives (peeking ahead).")
            iterat = [lift_s]
        if i == 1:
            # ROUND 2: Tấn công thực tế bằng cách thử các giá trị g khác nhau
        ...
        for g in iterat:
            ...
            if g == lift_s:
                if i == 0:
                    print(f"The real secret subs 1 mod q = {lift_s}")
                    print(f"pass {ctr} samples.")
                #break
...
# Run a simulation.
def shebang_PLWE(fval,qval,sval,numsampsval,numtrials,quickflag=False):
    global sig, n
    print("Welcome to the P-LWE Attack.")
    ...
    for trialnum in range(numtrials):
        ...
        print("********** PHASE 2: CREATE SECRET AND SAMPLES")
        # Tạo khóa bí mật secret và các mẫu P-LWE (a, b)
        create_secret_Pq()
        create_samples_Pq(numsampsval)
        ...
        for i in range(1,numsampsval):
            result = attack2_PLWE(i,500000000,quickflag)
...
shebang_PLWE(f, q, s, numsampsval=50, numtrials=1, quickflag=False)