...
# This file was *autogenerated* from the file PLWE.sage
from sage.all_cmdline import *   # import sage library

_sage_const_300 = Integer(300); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_10 = Integer(10); _sage_const_20 = Integer(20); _sage_const_80 = Integer(80); _sage_const_1000 = Integer(1000); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3); _sage_const_30 = Integer(30); _sage_const_10000 = Integer(10000); _sage_const_500000000 = Integer(500000000); _sage_const_1024 = Integer(1024); _sage_const_31 = Integer(31); _sage_const_3p192 = RealNumber('3.192'); _sage_const_50 = Integer(50)##################################################
# RING-LWE ATTACK #
##################################################
...
q = _sage_const_1                            # Modulo q
sig = _sage_const_1 /sqrt(_sage_const_2 *pi)              # Độ lệch chuẩn (sigma) cho nhiễu Gaussian - dummy values
Zq = IntegerModRing(q)          # Ring Z/qZ
R = PolynomialRing(Zq, names=('x',)); (x,) = R._first_ngens(1)# Ring đa thức trên Zq: R[x] = Zq[x]
f = y + _sage_const_1                        # Đa thức f
n = _sage_const_1                            # Bậc của đa thức f
N = NumberField(f, names=('a',)); (a,) = N._first_ngens(1)# Trường số được sinh bởi nghiệm của f là a; N = Q(a)
S = R.quotient(f, names=('z',)); (z,) = S._first_ngens(1)# Ring S[z] = R[z]/(f(z)) = Zq[z] / (f(z)) 
# => This is P_q
...
cmi = None
cm53 = cm.change_ring(RealField(_sage_const_10 )) # .change_ring - ép kiểu toàn bộ phần tử của cm về RealField(10)
...
secret = _sage_const_0                       # Vector bí mật (secret key)
samps = []                      # Danh sách các mẫu (a, b) = (a, a*s + e)
numsamps = _sage_const_1                     # Số lượng mẫu
lift_s = _sage_const_0                       # chuyển secret về dạng đa thức, tính giá trị đa thức tại x = 1, mod q.
...
def setup_params(fval, qval, sval):
    ...
    sig = sval/sqrt(_sage_const_2 *pi) # sigma - độ lệch chuẩn trong phân phối Gaussian
    S = R.quotient(f, names=('z',)); (z,) = S._first_ngens(1)# Zq[z]/f(z)
    ...
    if Mod(f.subs(y=_sage_const_1 ), q) != _sage_const_0 :
        raise ValueError(f"f(1) mod q = {Mod(f.subs(y=_sage_const_1 ), q)}, expected 0")
...
def cmatrix(): # returns a matrix, columns basis 1, x, x^2, x^3, ... given in the canonical embedding
    ...
    key = [_sage_const_0  for i in range(fdeg)] 
    ...
    while i < n:
        ...
        if Mod(i,_sage_const_20 )==Mod(_sage_const_0 ,_sage_const_20 ) or Mod(i,_sage_const_20 )==Mod(_sage_const_1 ,_sage_const_20 ):
        ...
        # Ta tách thành 2 hàng:
            ...
            key[i] = _sage_const_1       # Đánh dấu hàng phần thực: Real
            key[i+_sage_const_1 ] = _sage_const_2     # Đánh dấu hàng phần ảo: Imag
            ...
            for j in range(n):
                ...
                # M[i+1] = - Imag(val)
                M[i, j] = val.real()
                M[i+_sage_const_1 , j] = (val * I).real() 
            i = i + _sage_const_2 
...
def get_sample(latmat, latmatinv, sec, qval, keyval):
    ...
    dim = latmat.dimensions()[_sage_const_0 ]  # lấy số hàng của ma trận; cm[nxn]
...
def histoq(data):
    hist = [_sage_const_0  for i in range(_sage_const_10 )] # empty histogram
    zeroct = _sage_const_0  # count of zeroes mod q
    for datum in data:
        e = datum
        if e == _sage_const_0 : 
            zeroct += _sage_const_1 
        histbit = floor(ZZ(e)*_sage_const_10 /q)
        hist[histbit] = hist[histbit] + _sage_const_1 
    return [hist, zeroct]
...
def histogram_of_errors():
    ...
    errs = []
    for i in range(_sage_const_80 ):
        errs.append(sampler())
    hist = histo(errs,cmi)
    print("The number of error vectors that are zero:", hist[_sage_const_1 ])
    # bar_chart(hist[0], width=1).show(figsize=2)
    bar_chart(hist[_sage_const_0 ], width=_sage_const_1 ).save('histogram_of_errors.png') 
...
def histogram_of_as():
    ...
    a_vals = [samp[_sage_const_0 ] for samp in samps]
    hist = histo(a_vals,cmi)
    print("The number of a’s that are zero:", hist[_sage_const_1 ])
    # bar_chart(hist[0], width=1).show(figsize=2)
    bar_chart(hist[_sage_const_0 ], width=_sage_const_1 ).save('histogram_of_as.png') 
...
def histogram_of_errors_2():
    ...
    hist = histoq([ lift(Zq(go_to_q(sample[_sage_const_1 ],cmi) - go_to_q(sample[_sage_const_0 ],cmi)*go_to_q(secret,cmi))) for sample in samps])
    print("The number of such that are zero:", hist[_sage_const_1 ])
    # bar_chart(hist[0], width=1).show(figsize=2)
    bar_chart(hist[_sage_const_0 ], width=_sage_const_1 ).save('histogram_of_errors_2.png') 
...
def alg2(numsamps, reportrate, quickflag = False):
    ...
    b = [ _sage_const_0  for i in range(numsamps)]
    ...
    for i in range(numsamps):
        ...
        a[i] = go_to_q(sample[_sage_const_0 ],cmi) # a_i = a(1) mod q
        b[i] = go_to_q(sample[_sage_const_1 ],cmi) # b_i = b(1) mod q
        
    possibles = [] # Danh sách các ứng viên vượt qua toàn bộ mẫu
    winner = [[],_sage_const_0 ] # Lưu kẻ thắng cuộc: [[danh_sách_g], số_mẫu_vượt_qua]
    ...
    for i in range(_sage_const_2 ):
        ...
        for g in iterat:
            ...
            while ctr < numsamps and potential:
                ...
                # KIỂM TRA ĐIỀU KIỆN
                if e > q/_sage_const_4  and e < _sage_const_3 *q/_sage_const_4 :
                    potential = False 
                    # => kết thúc vòng lặp
                    # Cập nhật Winner
                    if ctr == winner[_sage_const_1 ]:
                        winner[_sage_const_0 ].append(g)
                ...
                ctr = ctr + _sage_const_1  # chuyển đến mẫu tiếp theo
            
            # Nếu g vượt qua toàn bộ các mẫu mà không bị loại
            if potential == True and i != _sage_const_0 :
    ...
    # KIỂM TRA CUỐI CÙNG
    if len(possibles) == _sage_const_1  and possibles[_sage_const_0 ] == lift_s:
        print("Success!")
        return True
    else:
        print("Failure!")
...
def shebang(fval,qval,sval,numsampsval,numtrials,quickflag=False):
    ...
    print("Quantity: ", RDF( qval/( _sage_const_2 *sqrt(_sage_const_2 )*sval*n*(qval-_sage_const_1 )**( (n-_sage_const_1 )/_sage_const_2 /n) ) ))
    ...
    for trialnum in range(numtrials):
        ...
        for i in range(_sage_const_1 ,_sage_const_30 ):
            result = alg2(i,_sage_const_10000 ,quickflag)
        ...
        if result == True:
            count_successes = count_successes + _sage_const_1 
            
        print("*~*~*~*~*~*~*~*~*~*~*~*~* ", count_successes, " out of ", trialnum+_sage_const_1 , " successes so far. *~*~*~*~*~*")
...
def secret_sub1_mod_q():
    ...
    lift_s = Zq(poly_in_ZZ_sec.subs(x=_sage_const_1 ))
    print("Storing the secret mod q.")
    print("The secret is ", secret)
    print("s(1) mod q = ", lift_s)
    return True
...
def attack2_PLWE(numsamps, reportrate, quickflag = False):
    ...
    for i in range(numsamps):
        ...
        poly_in_ZZ_a = sample[_sage_const_0 ].lift().change_ring(ZZ)
        a[i] = Zq(poly_in_ZZ_a.subs(x=_sage_const_1 ))
        
        poly_in_ZZ_b = sample[_sage_const_1 ].lift().change_ring(ZZ)
        b[i] = Zq(poly_in_ZZ_b.subs(x=_sage_const_1 ))
    ...
    for i in range(_sage_const_2 ):
        ...
        for g in iterat:
            ...
            if Mod(g,reportrate) == Mod(_sage_const_0 ,reportrate) and g != _sage_const_0 :
                print(f"Currently checking g = {g}")
...
def shebang_PLWE(fval,qval,sval,numsampsval,numtrials,quickflag=False):
    ...
    for trialnum in range(numtrials):
        ...
        for i in range(_sage_const_1 ,numsampsval):
            result = attack2_PLWE(i,_sage_const_500000000 ,quickflag)
...
# f(1) mod q = 0
f = y**_sage_const_1024  + _sage_const_2 **_sage_const_31  -_sage_const_2    
q = _sage_const_2 **_sage_const_31  -_sage_const_1 
# sigma = s/sqrt(2*pi)                 
s = _sage_const_3p192  

shebang_PLWE(f, q, s, numsampsval=_sage_const_50 , numtrials=_sage_const_1 , quickflag=False)
